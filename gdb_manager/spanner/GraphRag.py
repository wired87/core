...
from utils.ensembl import gene_info_url
...
class GotermRagProcess(SpannerEmbedder, SpannerAsyncHelper, SpannerRAG):
    def __init__(self, query, functionalities: List or None, user_id, job_id, term_limit: int = 25):
        ...
        Todo: Real time Graph (fetched data get converted to graph, embedded, given to llm)
    ...
    async def get_genes_from_gt(self, struct: list[dict]) -> list[dict]:
        """
        Given a list of structures with GO terms, fetch associated genes and return enriched data.
