...
class BQGroundZero:
    ...
    def __init__(self, dataset_id=None):
        ...
        self.ds_ref = f"{GCP_ID}.{BQ_DATASET_ID}"
        self.info_tid = BQ_INFO_TABLE_ID
    ...
    def upsert_row_query(self, table_id: str, rows: list[dict], schema: dict[str]) -> str:
        ...
        for row in rows:
            ...
            for k, v in row.items():
                # Handle different data types for correct BigQuery literal representation
                if isinstance(v, str):
    ...
    def add_col_query(self, col_name, table, col_type="STRING"):
        return f"""
    ...
    def schema_from_dict(self, json_data:dict, embed=None):
        """
...
class BQCore(BQGroundZero):
    ...
    def __init__(self, dataset_id=None):
        ...
        self.ensure_dataset_exists(ds_name=dataset_id)

        self.batch_upload_size = 1000

    def get_tables(self) -> List[str]:
        tables = []
        self.ensure_dataset_exists(ds_name=self.dataset)
    ...
    def update_bq_schema(self, keys, table):
    ...
    def bq_insert(self, table_id: str, rows: List[dict], schema):
        ...
        if len(rows) > 0:
            ...
            for i in range(0, len(rows), self.batch_upload_size):
                ...
                self.run_query(query)
                #self.bqclient.insert_rows_json(f"{self.pid}.{self.ds_id}.{table_id}", batch_chunk)
        else:
