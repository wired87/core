...
class GridStreamer:
    ...
    def put_frame(self, step: int, data: np.ndarray) -> None:
        """Non-blocking frame enqueue; drops oldest if queue is full."""
    ...
    async def _sender_loop(self) -> None:
        while self._running:
            try:
                _step, payload = await self._queue.get()
                try:
                    await asyncio.wait_for(self._send_fn(payload), timeout=self._send_timeout)
