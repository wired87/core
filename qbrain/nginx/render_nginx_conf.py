...
- NGINX_USE_HTTP_ONLY=true                  -> use HTTP-only template (no TLS, for container)
"""
import os
...
from qbrain.nginx.template import NGINX_TEMPLATE, NGINX_HTTP_ONLY_TEMPLATE
...
def _load_env_from_project_root() -> None:
    ...
    """
    try:
        base_dir = Path(__file__).resolve().parent  # qbrain/nginx
        project_root = base_dir.parent.parent  # project root (parent of qbrain)
...
def render() -> Path:
    ...
    use_http_only = os.environ.get("NGINX_USE_HTTP_ONLY", "").strip().lower() in ("true", "1", "yes")
    template = NGINX_HTTP_ONLY_TEMPLATE if use_http_only else NGINX_TEMPLATE
