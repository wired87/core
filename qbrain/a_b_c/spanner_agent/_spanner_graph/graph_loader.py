...
from google.cloud.spanner_admin_instance_v1 import Instance, InstanceConfig
from google.cloud.spanner_dbapi import exceptions


import time

from qbrain.a_b_c.spanner_agent import GCP_REGION, GCP_ID
from qbrain.a_b_c.spanner_agent._spanner_graph import ALL_INFO_SCHEMAS, SP_DATABASE_ID, create_default_auth_table_query, \
    create_default_node_table_query, create_default_edge_table_query, get_edge_table_schema, create_graph_query, \
    get_graph_query, get_col_names, get_graph_table_names_query, add_graph_table, create_metadata_table_query, \
    get_specific_g_info, SP_INSTANCE_ID
from qbrain.core.app_utils import ENV_ID
from qbrain.utils.run_subprocess import exec_cmd

"""Creates a database and tables for sample admin_data."""
...
class SpannerGroundZero:
    ...
    def add_table_schems_query(
            self,
            table_names,
            schema
    ) -> list:
        print("add_table_schems_query")
        # 1. Define all DDL statements to be executed
        ddl_statements = []

        # Format the new columns into a single string for DDL statement
        column_definitions = ", ".join(
            f"{column_name} {spanner_type}"
            for column_name, spanner_type in schema.items()
        )

        # 2. Create an ALTER TABLE statement for each table
        for table in table_names:
            ddl_statement = f"ALTER TABLE {table} ADD COLUMN {column_definitions}"
    ...
    def get_spanner_schema_from_dict(self, attrs):
        schema={}
        for key, val in attrs.items():
            col_type = self.get_spanner_type(val)
            schema[key] = col_type
        print("Spanner schema extracted")
        return schema
    ...
    def get_spanner_type(self, value):
        """
        Determines the Cloud Spanner-compatible admin_data type for a given Python value.
        :param value: The Python value to check.
        :return: The corresponding Cloud Spanner admin_data type.
        """
        if value is None:
            return "STRING(MAX)"
...
class SpannerCore(SpannerGroundZero):
    ...
    def __init__(self):
        ...
        self.instance_id = SP_INSTANCE_ID
        self.database_id = SP_DATABASE_ID
    ...
    def create_spanner_instance(self, instance_id: str, processing_units: int=100):
        """Creates a Spanner instance using gcloud with minimal parameters."""

        command = [
            "gcloud", "spanner", "instances", "create", instance_id,
            f"--processing-units={processing_units}",
            "--edition=STANDARD",
            f"--description={ENV_ID}"
        ]
        ...
        try:
            ...
            print(f"Error creating Spanner instance: {e.stderr}")
            raise
    ...
    def change_column_type(self, table_name, column_name, new_column_type):
        """
        Changes the admin_data type of a single column in a Google Cloud Spanner table.
        """
    ...
    # Angenommen, der Client ist bereits initialisiert: client = Client(project=self.project_id)

    def create_spanner_custom_config(
            self,
            config_id: str,
            display_name: str,
            base_region: str,  # Z.B. 'europe-west10'
            timeout_seconds: int = 300,
    ):
        """
        Erstellt eine neue USER_MANAGED Instanzkonfiguration in Spanner.

        Args:
            config_id (str): Die ID der neuen Konfiguration (Muss mit 'custom-' beginnen).
            display_name (str): Der Anzeigename.
            base_region (str): Die ID der Google-Managed Basiskonfiguration.
        """

        # 1. Erstelle die ARNs (vollständige Ressourcennamen)
        # custom_config_arn wird als 'name' im Objekt verwendet.
        custom_config_arn = f"projects/{self.project_id}/instanceConfigs/{config_id}"

        # base_config_arn wird als 'base_config' im Objekt verwendet.
        base_config_arn = f"projects/{self.project_id}/instanceConfigs/{config_id}"

        # 2. Erstelle das InstanceConfig-Objekt
        instance_config_to_create = InstanceConfig(
            # Name der neuen, benutzerdefinierten Konfiguration (MUSS der volle ARN sein)
            name=custom_config_arn,
            display_name=display_name,

            # Die zugrundeliegende Google-Managed Konfiguration (MUSS der volle ARN sein)
            base_config=base_config_arn,  # WAR ZUVOR AUSKOMMENTIERT! IST ABER PFLICHT!
        )

        # 3. API-Aufruf zur Erstellung
        print(f"Starte Erstellung der benutzerdefinierten Konfiguration '{config_id}'...")

        # Der API-Aufruf benötigt ZWEI KORREKTUREN:
        # A. Die 'instance_config_id' darf nur die KURZE ID sein (dein 'config_id').
        # B. Das Feld 'base_config' im Objekt muss enthalten sein.
        operation = self.client.instance_admin_api.create_instance_config(
            parent=f"projects/{self.project_id}",

            # 💥 REPARATUR 1: Muss die kurze ID sein. Die Variable 'config_id' ist hier korrekt.
            instance_config_id=config_id,

            instance_config=instance_config_to_create,
        )

        try:
            # Auf den Abschluss des Vorgangs warten
            new_config = operation.result(timeout=timeout_seconds)
            # Jetzt wird der volle ARN zurückgegeben
            print(f"Konfiguration '{new_config.name}' wurde erfolgreich erstellt.")
            return new_config.name  # Gebe den vollen ARN zurück zur Instanzerstellung
        except Exception as e:
            print(f"Fehler bei der Konfigurationserstellung: {e}")
            raise
    ...
    def check_create_spanner_instance(
            self,
            config_name,
            display_name,
            labels: dict = None,
        ...
        """
        Erstellt eine Cloud Spanner Instanz mit allen möglichen Konfigurationsparametern
        für die Rechenkapazität (entweder Nodes oder Processing Units).

        Args:
            project_id (str): Google Cloud Projekt ID.
            instance_id (str): Eindeutige ID für die neue Instanz.
            config_name (str): Der vollständige Name der Instanzkonfiguration (z.B. 'projects/PROJECT_ID/instanceConfigs/regional-europe-west1').
            display_name (str): Anzeigename der Instanz.
            node_count (int, optional): Anzahl der Knoten. Wenn gesetzt, wird 'processing_unit_count' ignoriert.
            processing_unit_count (int, optional): Anzahl der Processing Units. Wird nur verwendet, wenn 'node_count' None ist.
            labels (dict, optional): Schlüssel-Wert-Paare für GCP-Labels.
            timeout_seconds (int, optional): Timeout für den Erstellungsvorgang.

        Returns:
            Instance: Die erstellte Instance Objekt.
        """

        # 1. Instanz-Objekt erstellen und grundlegende Attribute festlegen
        instance = self.client.instance(self.instance_id)

        if instance.exists():
            print(f"Instanz '{self.instance_id}' existiert bereits.")
            return instance
        #google.spanner.admin.instance.v1.InstanceConfig
        base_cfg_name = f"projects/{self.project_id}/instanceConfigs/{display_name}"

        instance_cfg = self.create_spanner_custom_config(
            config_id=base_cfg_name,
            display_name=display_name,
            base_region=self.region,  # Z.B. 'europe-west1'
            timeout_seconds=300
        )


        # 3. Das eigentliche Instance-Objekt für die API-Anfrage erstellen
        instance_to_create = Instance(
            name=f"projects/{self.project_id}/instances/{self.instance_id}",
            display_name=display_name,
            config=instance_cfg.name,
            labels=labels if labels is not None else {},
            processing_units=100,
        )

        # 4. Asynchrone Erstellung der Instanz
        print(f"Starte Erstellung der Instanz '{self.instance_id}'...")

        # Die create() Methode gibt eine Operation zurück
        operation = self.client.instance_admin_api.create_instance(
            parent=f"projects/{self.project_id}",
        ...
        try:
            # Auf den Abschluss des Vorgangs warten
            instance_result = operation.result(timeout=timeout_seconds)
            print(f"Instanz '{self.instance_id}' wurde erfolgreich erstellt.")
            return instance_result
        except Exception as e:
            print(f"Fehler bei der Instanzerstellung: {e}")
            # Operation abbrechen oder Fehlerbehandlung hier einfügen
            raise
    ...
    def check_add_table(self, table_name, ttype: "auth" or "node" or "edge", schema_fetch=True):
        if not table_name or not ttype:
            print(f"Skipping malformed table definition")
            return
    ...
    def sp_create_database(self, instance_id, database_id, update_db=False):
        ...
        if update_db is True:
            # Update SpannerCore's database reference
            self.database = self.instance.database(database_id)
        return database

    def create_table(self, query, database=None):
        print(f"create table in {database}")
        database = database or self.database
        try:
            operation = database.update_ddl(ddl_statements=[query])
            operation.result()
        except Exception as e:
...
class SpannerAuth(SpannerCore):
    ...
    def get_user(self, email: str) -> Optional[dict]:
        ...
        :return: User admin_data dictionary if found, else None
        """
...
class SpannerGraphLoader(SpannerCore):
    def __init__(self, instance_id, db=None):
        super().__init__()
    ...
    def sadd_edge(self, src, trgt, attrs):
        ...
        if relationship:
            ...
            print(f"Edge {src} connected successfully to {trgt}.")
        else:
    ...
    def from_web(self, url):
        ...
        """
        # TODO: Implement web scraping and admin_data extraction logic
        pass
        ...
        with self.database.snapshot() as snapshot:
            if custom:
                results = snapshot.execute_sql(
                    ...
                    """,
                    params={
                        "embeds": None,#embed(str(admin_data)),
                        "limit": limit
                    },
                    param_types={
                        "admin_data": spanner.param_types.Array(spanner.param_types.FLOAT64),
                ...
                results = snapshot.execute_sql(
                    ...
                    LIMIT @limit;
                    """,
                    params={"admin_data": str(data), "limit": limit},
                    param_types={
                        "admin_data": spanner.param_types.STRING if not custom else None,
                        "limit": spanner.param_types.INT64
                    },
                )
