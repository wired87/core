...
from qbrain.a_b_c.spanner_agent._spanner_graph.graph_loader import SpannerGraphLoader
...
class SpannerGraphManager:
    ...
    def __init__(
        ...
        self.spg = SpannerGraphLoader(instance_id, database)
    ...
    async def add_schema_change_stream_table(
            self,
            node_id_map,
            node_schema,
            edge_schema,
        ...
        """
        Apply schema to change stream tables
        """
        try:
            # Add cols NODE TABLES
            table_schema_tasks = []
            for nid_table_name in node_id_map:
                table_schema_tasks.append(
                    self.spa.aadd_col(
                        keys=node_schema,
                        table=nid_table_name,
                        type_from_val=False,
                    )
                )

            # Add EDGE TABLE SCHEMA
            for eid in eid_map:
                table_schema_tasks.append(
                    self.spa.aadd_col(
                        keys=edge_schema,
            ...
            print("Schemas handled succesfully")
        except Exception as e:
            print(f"Err add_schema: {e}")
    ...
    async def abatch_commit(
        ...
        try:
            ...
            if create_session is True:
                await self.spa.acreate_session()
    ...
    def get_ids(self, table=None):
        ...
        if self.upload_to == "sp":
            ids = self.spa.get_all_ids(table)
        else:
            ids = self.spa.get_column_values(table, "id", )
    ...
    async def load_content(
        ...
        if testing is not None:
            ...
            if not os.path.exists(local_path):
                ...
                content = self.spa.structure_content_save(
                    content,
                    layer=layer,
                    save_to=local_path,
                    single=False
    ...
    def create_tables_batch(self, data):
        ...
        if self.upload_to == "sp":
            ...
            for k, v in data.items():
                self.spa.get_create_bq_table(
                    table_name=k,
                    ttype="edge" if any(c.islower() for c in k) else "node",
                )

    def get_g_universell_schema(self, g, combine_edge_and_node=False):
        """
        Extracts a universell spanner schema, uniform for all node types
        """

        node_schema = self.get_universell_sp_schema(g)
        edge_schema = self.extract_universell_edge_schema(g)
        return node_schema, edge_schema

    def get_schema_ntype(self, table_list, g):
        table_schema = {
            table: None
        ...
        for nid, attrs in g.G.nodes(data=True):
            ...
            if table_schema.get(ntype, None) is None:
                schema = {}
                for key, val in attrs.items():
                    col_type = self.spa.get_spanner_type(val)
                    schema[key] = col_type
                table_schema[ntype] = schema
        return table_schema


    def get_spanner_schema(self, attrs):
        print("Get spanner schema")
        schema = {}
        for k, v in attrs.items():
            if k not in schema:
                schema[k] = self.spa.get_spanner_type(v)
        print("RETURN SPANNER SCHEMA")
        return schema


    def extract_universell_edge_schema(self, g):
        edge_schema= {}
        for src, trgt, attrs in g.G.edges(data=True):
            if "src" not in edge_schema:
                col_type = self.spa.get_spanner_type(src)
                edge_schema["src"] = col_type
            if "trgt" not in edge_schema:
                col_type = self.spa.get_spanner_type(trgt)
                edge_schema["trgt"] = col_type

            for key, val in attrs.items():
                col_type = self.spa.get_spanner_type(val)
                edge_schema[key] = col_type
        return edge_schema
        ...
        for k, v in data.items():
            ...
            for row_key in all_keys:
                ...
                if sample_value is not None:
                    col_type = self.spa.get_spanner_type(sample_value)
                    """
                    else:
                    col_type = self.spa.get_bq_type(sample_value)
                    """
    ...
    def schema_batch_process(self, data):
        ...
        if self.upload_to == "sp":
            for k, v in data.items():
                self.spa.check_add_cols_batch(
                    keys=v["schema"],
                    t=k
                )
        else:
            for k, v in data.items():
                self.spa.update_bq_schema(
                    keys=v["schema"],
                    table=k
                )
    ...
    def table_schema_process(self, data, table_name, attrs, ttype="node"):
        if not data.get(table_name):
            ...
            schema = self.spa.check_add_table(table_name=table_name, ttype=ttype)
            # print("schema", schema)
            data[table_name] = schema

        for k, v in attrs.items():
            if k not in data.get(table_name):
                spanner_type = self.spa.get_spanner_type(v)
                self.spa.check_add_cols(
        ...
        for k, v in attrs.items():
            # print("kv", k, v)
            if k not in data.get(table_name):
                data[table_name].update({k: self.spa.get_spanner_type(v)})

    ############ SPANNER
    async def atable_schema_process(self, data, table_name: str, attrs: Dict, ttype: str = "node"):
        if not data.get(table_name):
            # create table, return its default schema (if new created), update local
            schema = await self.spa.acheck_add_table(table_name=table_name, ttype=ttype)
            # print("schema", schema)
            data[table_name] = schema
        try:
            if self.upload_to == "sp":
                schema = await self.spa.aadd_col(keys=attrs, table=table_name, existing_schema=data[table_name])
                # print("table schema", schema)
            else:
                schema = await self.spa.update_bq_schema(keys=attrs, table=table_name)
