...
from qbrain.a_b_c.spanner_agent._spanner_graph import SP_DATABASE_ID, create_default_auth_table_query, \
    create_default_node_table_query, create_default_edge_table_query
from qbrain.a_b_c.spanner_agent._spanner_graph.graph_loader import SpannerCore
from qbrain.core.app_utils import GCP_ID


class ASpannerManager(SpannerCore):
    """
    If you need to change the structure of your database
    (add a column, create a table, etc.), use the DatabaseAdminAsyncClient.

    If you need to read or write admin_data within your existing tables,
    ...
    def get_spanner_db_path(self, dbid):
        return f"projects/{GCP_ID}/instances/brainmaster/databases/{dbid}"


    async def acreate_session(self, dbid:str=SP_DATABASE_ID):
        ...
        try:
            db_path = self.get_spanner_db_path(dbid)
            request = CreateSessionRequest(database=db_path)
    ...
    async def asnap(self, query, return_as_dict=False):
        ...
        if self.session is None:
            self.session = await self.acreate_session()
        request = ExecuteSqlRequest(session=self.session.name, sql=query)
...
async def main(spanner_helper=None, instance_id=None):
    spanner_helper = spanner_helper or ASpannerManager(instance_id)
