...
class ServerWorker:
    ...
    Acts as Head Node
    initialize ENV
    Waits for web request from relay (b1) with credentials
    """
    ...
    async def handle_all_workers_active(self):
        ...
        # Prepare trainer todo
        #self.all_qfns = [(nid, attrs) for nid, attrs in self.g.G.nodes(data=True) if attrs.get("type") == "QFN"]
