"""
Modular visualizer: adaptable to cfg specifications and data keys.
...
class ModularVisualizer:
    ...
    def _reshape_time_db(self, time_db_flat: np.ndarray) -> list[list[list[np.ndarray]]]:
        ...
        return _reshape_flat_db(flat, param_ctrl, amount_per_field, n_modules, n_fields)

    def _extract_data_points(
        self, reshaped: list[list[list[np.ndarray]]], positions: list[tuple[int, ...]]
    ) -> np.ndarray:
        points = []
        for mod in reshaped:
            for field in mod:
    ...
    def _extract_positions_and_values(self, time_db_flat: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    def plot_time_step(self, time_db: list[np.ndarray] | np.ndarray, time_idx: int = 0, ax=None, **kwargs):
