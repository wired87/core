...
class Guard:
    ...
    def run(self, cfg: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if not cfg:
            print("[jax_test.grid.Guard] No cfg provided, skipping workflow")
            return {}
    ...
    def _validate_cfg(self, cfg: dict) -> None:
        ...
        for k in required:
            if k not in cfg:
                raise ValueError(f"[jax_test.grid.Guard] cfg missing required key: {k}")
    ...
    def _decode_db(self, cfg: dict) -> np.ndarray | None:
        ...
        try:
            ...
            print(f"[jax_test.grid.Guard] DB decode warning: {e}")
            return None
    ...
    def _run_workflow(self, cfg: dict) -> dict:
        ...
        """
        print("[jax_test.grid.Guard] Running workflow with cfg keys:", list(cfg.keys()))
        ...
        print(f"[jax_test.grid.Guard] Model artifact written to {model_path}")

        npz_path = model_path.replace(".json", "_data.npz")
        if db_flat is not None:
            np.savez_compressed(npz_path, db_flat=db_flat)
            print(f"[jax_test.grid.Guard] Model data written to {npz_path}")
