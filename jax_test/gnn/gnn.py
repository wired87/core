...
class GNN(
    ...
    def main(
        ...
        self._grid_streamer = grid_streamer
        self._grid_animation_recorder = grid_animation_recorder
    ...
    def simulate(self):
        for step in range(self.glob_time):
            ...
            if self._grid_streamer is not None:
                try:
                    import numpy as np
                    nodes_arr = np.asarray(new_g.nodes).astype(np.float32).ravel()
                    self._grid_streamer.put_frame(step, nodes_arr)
                except Exception:
                    pass
            if self._grid_animation_recorder is not None:
                try:
                    import numpy as np
                    nodes_arr = np.asarray(new_g.nodes).astype(np.float32).ravel()
                    self._grid_animation_recorder.save_frame(step, nodes_arr)
                except Exception:
                    pass
        ...
        if self._grid_animation_recorder is not None:
            try:
                self._grid_animation_recorder.finish()
            except Exception:
                pass
...
def run_all_chains(chains, old_g_batch, new_g_batch, active_pattern_map):
    ...
    vmap = nnx.vmap(
        ...
        in_axes=(
            ...
            None,
            None,
            None, # overall same index (per calc) -< todo wasnt able to handle dmu within the calc process but currently the system ensures dynamic node updates based on energetc activity
        )
    )
