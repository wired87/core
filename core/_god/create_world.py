...
from core._god.env_node_creator import EnvNodeCreator
from core.data import QF_LEX
from core.qf_utils.all_subs import ALL_SUBS, G_FIELDS, H, FERMIONS
from core.qf_utils.mover import Mover
...
class God:
    ...
    def __init__(
        ...
        self.env_creator = EnvNodeCreator(
            env_id=ENV_ID,
        )
    ...
    def create_env(self):
        # Create ENV PARAMs for qenv
        self.g.add_node(
            attrs=dict(
                nid="ENV",
                type="MODULE",
                value=EnvNodeCreator(
                    env_id=ENV_ID,
                    world_cfg=None
                ).create()
            )
    ...
    def load_attrs_in_sp(self):
        """
        CPU data creation - create spanner
        """
