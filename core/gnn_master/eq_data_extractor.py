...
from app_utils import DIM, USER_ID, ENV_ID
...
class NodeDataProcessor(
    ...
    """
    process all data send inc generated pattern in batch to gpu
    """
    ...
    def set_axis(self,data):
        """
        Determines the vmap axis for each parameter in the data bundle.
        - Use axis 0 for array-like data (map over it).
        - Use None for scalar data (broadcast it).
        """
        self.axis_def = tuple(
            0
    ...
    def build_processor_module(
        ...
        data:dict = self.qfu.batch_field_single(
            ntype=self.ntype,
            amount_nodes=1, # create xample data
            dim=DIM,
        )
    ...
    def set_pattern(self, ):
        param_struct = [
            [] for _ in range(len(self.arsenal_struct))
        ]

        for item in self.arsenal_struct:
            param_eq_node = self.g.G.nodes[
                item["nid"]]
            params = item["params"]
    ...
    def set_edges_fields(
        ...
        """
        Set interactants for all enighbors
        """
    ...
    def upsert_bq(self):
        ...
        extend bq table with entire data structs.
        ...
        for attrs in self.attrs_struct:
            ...
            # bq format -> ensure all data fits into single table
